...
Изображения из директории assets в результате сборки будут перемещены в static/img.lazy/assets/имя_компонента или static/img.lazy/assets/имя_компонента/имя_вложенного_компонента, если у компонент вложен в другой компонент. К картинкам относятся: svg, png, jpg, jpeg, jpe, gif, tiff и bmp. Все остальные файлы будут помещены в папку components-assets (имя папки зависит от опции fs.componentsFolderName).
...
static/                                     # Папка для статики. Название для папки настраивается в tars-config.js
    ...
    ├── img.lazy/                                # Картинки. Название для папки настраивается в tars-config.js
